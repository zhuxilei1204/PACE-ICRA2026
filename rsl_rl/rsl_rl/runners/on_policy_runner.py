...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        # serve/success tracking (for TTEnv-style tasks)
        self._serve_total: int = 0
        self._success_total: int = 0
        try:
            # per-env accumulator: whether this ongoing serve has achieved success
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # track table-tennis serve success statistics if available
                    try:
                        # Update per-env success flag during ongoing serve
                        if (
                            hasattr(self.env, "has_touch_opponent_table_just_now")
                            and hasattr(self.env, "has_touch_paddle")
                            and getattr(self, "_serve_success_flag", None) is not None
                        ):
                            event_mask = (self.env.has_touch_opponent_table_just_now & self.env.has_touch_paddle)
                            self._serve_success_flag |= event_mask.to(self._serve_success_flag.device)

                        # On serve boundary (ball reset), aggregate and reset flags per env
                        if hasattr(self.env, "ball_reset_ids") and self.env.ball_reset_ids is not None:
                            ids = self.env.ball_reset_ids
                            if isinstance(ids, torch.Tensor) and ids.numel() > 0 and getattr(self, "_serve_success_flag", None) is not None:
                                ids_dev = ids.to(self._serve_success_flag.device)
                                self._serve_total += int(ids_dev.numel())
                                self._success_total += int(self._serve_success_flag[ids_dev].sum().item())
                                # reset flags for new serves
                                self._serve_success_flag[ids_dev] = False
                    except Exception:
                        pass
            ...
            if self.log_dir is not None and not self.disable_logs:
                # Augment logs with serve success rate if tracked
                loss_dict = dict(loss_dict)
                if self._serve_total > 0:
                    loss_dict["serve_success_rate_total"] = float(self._success_total) / float(max(1, self._serve_total))
                # Log information
                self.log(locals())
                # Save model
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        eta_seconds = self.tot_time / max(1, (locs['it'] - locs['start_iter'] + 1)) * (
            locs['start_iter'] + locs['num_learning_iterations'] - locs['it']
        )
        # format ETA with days if needed
        if eta_seconds >= 86400:
            d = int(eta_seconds // 86400)
            h = int((eta_seconds % 86400) // 3600)
